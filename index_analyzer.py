...
class IndexAnalyzer:
    ...
    def get_index_valuation_percentile(self, index_code: str, date: str, years: int = 5) -> dict:
        """
        ...
        """
        start_date = (pd.to_datetime(date) - pd.Timedelta(days=365*years)).strftime('%Y%m%d')
        df_index_basic = self.dm.pro.index_dailybasic(ts_code=index_code, start_date=start_date, end_date=date)
