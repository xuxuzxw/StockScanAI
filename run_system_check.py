...
def run_all_checks():
    ...
    try:
        assert config.TUSHARE_TOKEN and config.TUSHARE_TOKEN != "YOUR_TUSHARE_TOKEN", "TUSHARE_TOKEN 未配置"
        assert config.DATABASE_URL, "DATABASE_URL 未配置"
        assert config.AI_MODEL_CONFIG['fast_and_cheap']['api_key'], "AI_API_KEY_FAST 未配置"
    ...
    try:
        ...
        log.info(f"  [PASS] 成功连接到数据库: {config.DB_HOST}:{config.DB_PORT}/{config.DB_NAME}")
        checks_passed += 1
    except Exception as e:
        log.error(f"  [FAIL] 数据库连接失败: {e}", exc_info=True)
        log.critical("无法连接到数据库，后续检查将失败。请检查数据库服务是否运行，以及.env中的连接信息是否正确。")
        finalize_report(checks_passed, total_checks)
        return
    ...
    try:
        ...
        assert stock_list is not None and not stock_list.empty, "获取股票列表失败，返回为空。"
        log.info(f"  [PASS] DataManager 实例化成功，并成功从 Tushare 获取到 {len(stock_list)} 只上市股票的基本信息。")
        checks_passed += 1
    except Exception as e:
    ...
    try:
        ...
        trade_cal = dm.pro.trade_cal(exchange='', start_date='20240101', end_date=today_str)
        ...
        assert isinstance(pe_ttm, float) and pd.notna(pe_ttm), f"为 {ts_code_test} 计算的 pe_ttm 无效。"
        log.info(f"  [PASS] 因子引擎实例化成功，并为 {ts_code_test} 计算出 PE(TTM) 因子值为: {pe_ttm:.2f}")
        checks_passed += 1
    except Exception as e:
    ...
    try:
        ...
        log.info("  [PASS] AI引擎实例化成功，并成功调用低成本模型 API，密钥和网络连接正常。")
        checks_passed += 1
    except Exception as e:
        log.error(f"  [FAIL] AI层检查失败: {e}", exc_info=True)
        log.warning("请检查 .env 文件中的 AI_API_KEY_FAST 是否正确，以及网络是否能访问AI服务。")
        finalize_report(checks_passed, total_checks)
        return
    ...
    try:
        ...
        test_adaptive_strategy = qe.AdaptiveAlphaStrategy(ff, fp, qe.FactorAnalyzer(dm), pd.DataFrame())
        test_risk_manager = qe.RiskManager(ff, fp)
        assert test_adaptive_strategy is not None
        assert test_risk_manager is not None
        log.info("  [PASS] 每日策略工作流的核心组件均可被成功实例化，端到端流程基本通畅。")
        checks_passed += 1
    except Exception as e:
