...
def run_main_workflow():
    ...
    try:
        ...
        if not portfolio_weights.empty:
            ...
            prices = data_manager.get_adjusted_daily(
                target_stock, backtest_start, backtest_end
            )
            ...
            if not all_prices.empty:
                ...
                log.info(
                    f"从 {backtest_start} 到 {backtest_end} 的持有期累计收益率为: {cumulative_return:.2%}"
                )
            else:
        ...
        log.critical(
            "主控流程发生严重错误!", exc_info=True
        )  # exc_info=True会记录完整的错误堆栈
...
class FactorFactory:
    ...
    def calc_net_inflow_ratio(
        self, ts_code: str, start_date: str, end_date: str, window=5
    ) -> float:
        """计算N日净流入额占成交额比例"""
        ...
        trade_amount = (
            recent_flow["buy_lg_amount"].sum()
            + recent_flow["sell_lg_amount"].sum()
            + recent_flow["buy_md_amount"].sum()
            + recent_flow["sell_md_amount"].sum()
            + recent_flow["buy_sm_amount"].sum()
            + recent_flow["sell_sm_amount"].sum()
        )
        return net_mf_amount / trade_amount if trade_amount != 0 else 0

    def calc_north_hold_change(
    ...
    def calc_roe(self, ts_code: str, date: str, **kwargs) -> float:
        ...
        try:
            ...
            latest_fina = self.data_manager.get_pit_financial_data(
                all_fina, as_of_date=date
            )
    ...
    def calc_growth_revenue_yoy(self, ts_code: str, date: str, **kwargs) -> float:
        ...
        try:
            ...
            # 优先使用 'or_yoy' 字段
            if "or_yoy" in latest_fina_row.index and pd.notna(
                latest_fina_row["or_yoy"]
            ):
                return latest_fina_row["or_yoy"]

            # 回退到 'netprofit_yoy'
            if "netprofit_yoy" in latest_fina_row.index and pd.notna(
                latest_fina_row["netprofit_yoy"]
            ):
    ...
    def calc_holder_num_change_ratio(self, ts_code: str, date: str, **kwargs) -> float:
        ...
        available_data = df_holder_num[
            df_holder_num["ann_date"] <= as_of_date_dt
        ].copy()
    ...
    def calc_major_shareholder_net_buy_ratio(
        self, ts_code: str, date: str, lookback_days=90, **kwargs
    ) -> float:
        """【V2.1新增】计算近N日重要股东增减持金额占自由流通市值的比例(%)。"""
        end_date_dt = pd.to_datetime(date)
        start_date_str = (end_date_dt - pd.Timedelta(days=lookback_days)).strftime(
            "%Y%m%d"
        )
        end_date_str = end_date_dt.strftime("%Y%m%d")

        df_trade = self.data_manager.get_holder_trade(
        ...
        net_buy_value = (
            df_trade[df_trade["in_de"] == "IN"]["change_value"].sum()
            - df_trade[df_trade["in_de"] == "DE"]["change_value"].sum()
        )

        # 获取最新的流通市值
        df_basic = self.data_manager.get_daily_basic(
            ts_code, end_date_str, end_date_str
        )
        ...
        float_cap = (
            df_basic["float_share"].iloc[0] * df_basic["close"].iloc[0]
        )  # 单位: 万*元
        if float_cap == 0:
            return np.nan
    ...
    def calc_top_list_net_buy_amount(
        self, ts_code: str, date: str, top_list_df: pd.DataFrame
    ) -> float:
    ...
    def calc_forecast_growth_rate(self, ts_code: str, date: str, **kwargs) -> float:
        ...
        end_date_dt = pd.to_datetime(date)
        start_date_hist = (end_date_dt - pd.Timedelta(days=365)).strftime(
            "%Y%m%d"
        )  # 回看一年内的预告
        ...
        available_forecast = df_forecast[
            df_forecast["ann_date"] <= end_date_dt
        ...
        # 取预告增长率的中间值
        avg_growth = (
            latest_forecast["p_change_min"] + latest_forecast["p_change_max"]
        ) / 2
        return avg_growth

    def calc_repurchase_ratio(
        self, ts_code: str, date: str, lookback_days=365, **kwargs
    ) -> float:
        ...
        df_repurchase = self.data_manager.get_repurchase(
            ts_code, start_date_str, end_date_str
        )
    ...
    def calc_top10_holder_ratio(self, ts_code: str, date: str, **kwargs) -> float:
        ...
        for year_offset in range(3):  # 回看3年
            ...
            for p in periods:
                ...
                df_holders = self.data_manager.get_top10_floatholders(
                    ts_code=ts_code, period=p
                )

                if (
                    df_holders is not None
                    and not df_holders.empty
                    and "ann_date" in df_holders.columns
                ):
                    df_holders["ann_date"] = pd.to_datetime(
                        df_holders["ann_date"], format="%Y%m%d"
                    )
                    # 查找在目标日期或之前已公告的数据
    ...
    def calc_m1_m2_scissors_gap(self, date: str, **kwargs) -> float:
        ...
        """
        target_date = pd.to_datetime(date, format="%Y%m%d")
        start_m = (target_date - pd.Timedelta(days=90)).strftime(
            "%Y%m"
        )  # 获取近3个月的数据确保覆盖
    ...
    def create_event_dummy_factor(self, extracted_events: str) -> int:
        ...
        if any(
            keyword in extracted_events for keyword in ["增持", "新品发布", "业绩增长"]
        ):
            return 1
        elif any(
            keyword in extracted_events for keyword in ["调查", "诉讼", "监管提示"]
        ):
            return -1
        return 0
    ...
    def _get_price_series(
        self, ts_code: str, end_date: str, lookback_days: int
    ) -> pd.Series | None:
        """获取用于计算技术指标的后复权价格序列的辅助函数"""
        start_date = (
            pd.to_datetime(end_date) - pd.Timedelta(days=lookback_days)
        ).strftime("%Y%m%d")
        df_adj = self.data_manager.get_adjusted_daily(
            ts_code, start_date, end_date, adj="hfq"
        )
...
class FactorProcessor:
    ...
    def _neutralize_industry(
        self, factor_series: pd.Series, stock_basics: pd.DataFrame
    ) -> pd.Series:
        """行业中性化处理"""
        df = pd.DataFrame({"factor": factor_series})
        df_basics_indexed = stock_basics.set_index("ts_code")
        df = pd.merge(
            df, df_basics_indexed[["industry"]], left_index=True, right_index=True
        )
        ...
        try:
            ...
            # OLS: beta = (X'X)^-1 * X'y
            beta = (
                np.linalg.inv(X_with_const.T @ X_with_const) @ X_with_const.T @ y.values
            )
    ...
    def process_factor(self, factor_series: pd.Series, neutralize=True) -> pd.Series:
        ...
        if neutralize:
            ...
            relevant_stocks_info = all_stocks_info[
                all_stocks_info["ts_code"].isin(processed_series.index)
            ]
            processed_series = self._neutralize_industry(
                processed_series, relevant_stocks_info
            )
...
class FactorAnalyzer:
    ...
    def load_factors_from_db(self, table_name: str) -> pd.DataFrame:
        ...
        try:
            df = pd.read_sql(
                f"SELECT * FROM {table_name}", self.conn, index_col="trade_date"
            )
    ...
    def calculate_ic(self, factor_series: pd.Series, forward_return_period=20) -> tuple:
        ...
        end_date = start_date + pd.Timedelta(
            days=forward_return_period * 2
        )  # 预留足够的时间窗口

        prices_df = pd.DataFrame()
        for stock in stock_list:
            prices = self.data_manager.get_adjusted_daily(
        ...
        # 2. 计算未来收益率
        future_returns = prices_df.pct_change(periods=forward_return_period).shift(
    ...
    def run_layered_backtest(
        self, factor_df: pd.DataFrame, num_quantiles=5, forward_return_period=20
    ) -> tuple:
        """
        执行分层回测，评估因子的单调性。
        ...
        end_date = (
            factor_df.index.max() + pd.Timedelta(days=forward_return_period * 2)
        ).strftime("%Y%m%d")
        ...
        for date in rebalance_dates:
            ...
            try:
                quantiles = pd.qcut(
                    factor_slice, num_quantiles, labels=labels, duplicates="drop"
                )
            ...
            period_end_idx = min(
                period_start_idx + forward_return_period, len(daily_returns)
            )
...
class AlphaStrategy:
    ...
    def generate_scores(
        self, ts_code: str, date: str, ai_analysis_results: dict = None
    ) -> dict:
        """为单个股票生成综合评分。"""
        scores = {}
        ...
        pe_score = (
            100 - min(pe_factor, 100) if pd.notna(pe_factor) and pe_factor > 0 else 50
        )
        ...
        scores["reasoning"] = (
            f"量化综合评分: {quant_score:.2f} (PE贡献: {pe_score*0.4:.2f}, ROE贡献: {roe_score*0.6:.2f})."
        )

        if ai_analysis_results:
            event_factor = self.factor_factory.create_event_dummy_factor(
                ai_analysis_results.get("key_events", "")
            )
            if event_factor == -1:
            ...
            try:
                ...
                sentiment_score = 0.0
            sentiment_factor = self.factor_factory.create_sentiment_factor(
                sentiment_score
            )
...
class AdaptiveAlphaStrategy:
    ...
    def __init__(
        self, factor_factory, factor_processor, factor_analyzer, stock_prices_df
    ):
        self.factor_factory = factor_factory
        self.factor_processor = factor_processor
        self.factor_analyzer = factor_analyzer
        self.stock_prices_df = (
            stock_prices_df  # 需要全周期的价格数据来计算因子和未来收益
        )

    @lru_cache(maxsize=16)  # 缓存计算过的权重，提高重复调用速度
    def calculate_dynamic_weights(
        self, end_date_str: str, lookback_days: int, factors_to_use: tuple
    ) -> pd.Series:
        """
        ...
        trade_dates_in_period = all_trade_dates[
            (all_trade_dates >= start_date) & (all_trade_dates <= end_date)
        ]

        # 我们按月计算IC，以平衡计算量和时效性
        monthly_dates = (
            trade_dates_in_period.to_period("M").to_timestamp("M", how="end").unique()
        )
        ...
        for factor_name in factors_to_use:
            ...
            for date in monthly_dates:
                    ...
                    for s in prices_slice.columns:
                        ...
                        if factor_name == "momentum":
                            ...
                            if len(df_temp) >= 20:
                                ...
                                ).iloc[-20:].std() * np.sqrt(252)
                else:  # 其他非价格序列因子
                    raw_values = {
                        s: getattr(self.factor_factory, f"calc_{factor_name}")(
                            s, factor_start_date, date_str
                        )
                        for s in stock_pool
                    }
                ...
                processed_factor = self.factor_processor.process_factor(
        ...
        # 计算IR作为权重
        ir_values = {
            name: self.factor_analyzer.calculate_ir(ics)
    ...
    def generate_composite_factor(
        ...
        dynamic_weights = self.calculate_dynamic_weights(
            trade_date.strftime("%Y%m%d"), ic_lookback_days, factors_to_use
        )
        ...
        for factor_name, weight in dynamic_weights.items():
            ...
            if factor_name in ["momentum", "volatility"]:
                ...
                for s in prices_slice.columns:
                    ...
                    if factor_name == "momentum":
                        if len(df_temp) >= 20:
                            raw_values[s] = (
                                df_temp["close"].iloc[-1] / df_temp["close"].iloc[-20]
                                - 1
                            )
                    elif factor_name == "volatility":
                        if len(df_temp) >= 20:
            ...
            if composite_factor.empty:
                composite_factor = (
                    processed_factor.mul(weight).reindex(stock_pool).fillna(0)
                )
            else:
...
class PortfolioOptimizer:
    ...
    def optimize_max_sharpe(self, max_weight_per_stock=0.1):
        ...
        if not opts.success:
            ...
            return pd.DataFrame(
                weights, index=self.expected_returns.index, columns=["weight"]
            )

        optimized_weights = pd.DataFrame(
            opts.x, index=self.expected_returns.index, columns=["weight"]
        )
...
class VectorizedBacktester:
    ...
    def run(self, weights_df: pd.DataFrame = None):
        ...
        if len(rebalance_dates) < 2:
            ...
            self.results = {
                "performance": pd.Series({"错误": "有效调仓日不足2个"}),
        ...
        for i in range(len(rebalance_dates) - 1):
            ...
            turnover = (
                (
                    target_weights.subtract(
                        last_weights.reindex(target_weights.index, fill_value=0),
                        fill_value=0,
                    )
                )
                .abs()
            ...
            period_daily_returns = (
                period_prices[target_weights.index].pct_change().iloc[1:]
            )

            if self.stop_loss_pct is not None:
                cumulative_period_returns = (1 + period_daily_returns).cumprod()
                stop_loss_triggered = (
                    cumulative_period_returns < (1 - self.stop_loss_pct)
                ).any(axis=1)
            ...
            weighted_returns = period_daily_returns.dot(
                target_weights.reindex(period_daily_returns.columns, fill_value=0)
            )
    ...
    def _calculate_performance(self):
        ...
        annual_return = (
            cumulative_returns.iloc[-1] ** (trading_days_per_year / total_days) - 1
        )
    ...
    def plot_results(self):
        """绘制回测结果图表"""
        if (
            "cumulative_returns" not in self.results
            or self.results["cumulative_returns"].empty
        ):
        ...
        fig.add_trace(
            go.Scatter(
                x=self.results["cumulative_returns"].index,
        ...
        drawdown = (
            ...
            - self.results["cumulative_returns"].cummax()
        ) / self.results["cumulative_returns"].cummax()
        fig.add_trace(
            go.Scatter(
                x=drawdown.index,
...
class AutomatedTasks:
    ...
    def run_daily_data_update(self):
        ...
        try:
            ...
            self.data_manager.get_daily(
                "000001.SZ", "20200101", datetime.now().strftime("%Y%m%d")
            )
    ...
    def run_factor_calculation(self, output_path="data/processed_factors.csv"):
        ...
        try:
            ...
            start_date = (pd.to_datetime(latest_date) - pd.Timedelta(days=50)).strftime(
                "%Y%m%d"
            )
...
class MarketProfile:
    ...
    def get_market_regime(self, end_date: datetime) -> str:
        ...
        if pmi_col not in latest_pmi.index:
            log.warning(
                f"无法在PMI数据中找到 '{pmi_col}' 列。可用列: {latest_pmi.index.tolist()}"
            )
            return "未知(数据列缺失)"
...
class MLAlphaStrategy:
    ...
    def _prepare_data(
        ...
        # 1. 计算未来收益率作为标签 (y)
        future_returns = all_prices.pct_change(periods=forward_return_period).shift(
            -forward_return_period
        )
        ...
        log.info(
            f"【ML数据准备】数据准备完毕。共 {len(X_final)} 个样本，{len(self.features_columns)} 个特征。"
        )
    ...
    def predict_top_stocks(
        self, factor_snapshot: pd.DataFrame, top_n: int = 20
    ) -> pd.Index:
        """
        ...
        try:
            ...
            log.error(
                f"特征对齐失败: {e}。模型需要特征: {self.model.feature_name_}，但输入数据只有: {factor_snapshot.columns.tolist()}"
            )
            return pd.Index([])
...
class RiskManager:
    ...
    def __init__(
        self, factor_factory: FactorFactory, factor_processor: FactorProcessor
    ):
    ...
    def calculate_risk_exposure(
        self, portfolio_weights: pd.Series, trade_date: str
    ) -> pd.Series:
        """
        计算给定投资组合在指定日期的风险暴露。
        ...
        for factor in self.risk_factors:
            params = {
                "date": trade_date,
                "start_date": (
                    pd.to_datetime(trade_date) - pd.Timedelta(days=365)
                ).strftime("%Y%m%d"),
                "end_date": trade_date,
            }
            raw_values = {
                code: self.ff.calculate(factor, ts_code=code, **params)
            ...
            if not raw_series.empty:
                processed_series = self.fp.process_factor(
                    raw_series, neutralize=False
                )  # 风险因子通常不进行行业中性化
        ...
        # 3. 计算投资组合的加权风险暴露
        # 将持仓权重与股票的风险因子暴露对齐
        aligned_weights, aligned_risk_df = portfolio_weights.align(
            risk_df, join="inner", axis=0
        )
...
class PerformanceAttribution:
    ...
    def _aggregate_by_industry(self, weights, returns):
        """按行业聚合权重和收益的辅助函数。"""
        df = pd.DataFrame({"weight": weights, "return": returns}).join(
            self.stock_industry_map
        )
        industry_return = df.groupby("industry").apply(
    ...
    def optimize(self, max_weight_per_stock=0.1, industry_max_exposure=0.2, gamma=0.5):
        ...
        for industry in industries:
            industry_stocks_idx = [
                ...
                if self.stock_industry_map.loc[t, "industry"] == industry
            ]
            if industry_stocks_idx:
                constraints.append(
                    cp.sum(weights[industry_stocks_idx]) <= industry_max_exposure
                )
        ...
        try:
            ...
            optimized_weights = pd.DataFrame(
                weights.value, index=self.tickers, columns=["weight"]
            )
...
class DataHandler(ABC):
    ...
    @abstractmethod
    def get_latest_bar(self, ts_code):
        pass
...
class SimplePortfolio(Portfolio):
    ...
    def __init__(
        ...
        self.current_positions = {
            code: 0 for code in self.data_handler.all_prices.columns
        }
    ...
    def update_signal(self, event: SignalEvent):
        ...
        if event.direction == "LONG":
            ...
            if self.current_holdings["cash"] < target_value:
                log.debug(
                    f"[{self.data_handler.latest_data['timestamp'].strftime('%Y-%m-%d')}] 跳过买入 {event.ts_code}: 现金不足。"
                )
                return
            ...
            if quantity > 0:
                ...
                log.info(
                    f"[{self.data_handler.latest_data['timestamp'].strftime('%Y-%m-%d')}] 生成买入订单: {event.ts_code}, 数量: {quantity}"
                )
    ...
    def get_performance_report(self) -> dict:
        ...
        annual_return = (1 + cumulative_return) ** (
            trading_days_per_year / total_days
        ) - 1
...
class MockExecutionHandler(ExecutionHandler):
    ...
    def execute_order(self, event: OrderEvent):
        ...
        # 2. 市场冲击滑点 (与交易量和流动性相关)
        avg_vol_20d = self.data_handler.get_average_volume(
            event.ts_code, 20, self.data_handler.latest_data["timestamp"]
        )
...
class SimpleDataHandler(DataHandler):
    ...
    def __init__(
        self, events_queue, all_prices_df: pd.DataFrame, all_volumes_df: pd.DataFrame
    ):
        self.events = events_queue
        # 确保时间索引是 datetime 类型
...
class SimpleMovingAverageStrategy(Strategy):
    ...
    def __init__(
        self, data_handler: SimpleDataHandler, short_window=10, long_window=30
    ):
        self.data_handler = data_handler
    ...
    def calculate_signals(self, event: MarketEvent):
        ...
        for code in self.data_handler.all_prices.columns:
            ...
            hist_prices = self.data_handler.get_historical_bars(
                code, self.long_window, event.timestamp, bar_type="price"
            )
...
class IndustryRotationStrategy:
    ...
    def select_portfolio(
        ...
        """
        # 1. 行业筛选
        top_industries = self.ia.get_industry_factor_rank(date, industry_factor).head(
            top_n_industries
        )
        log.info(f"选出的Top行业:\n{top_industries}")

        # 2. 在选定行业内进行个股选择
        stock_pool_df = self.ia.stock_basics[
            self.ia.stock_basics["industry"].isin(top_industries.index)
        ]
        ...
        for code in stock_pool_df["ts_code"]:
            try:
                stock_factor_values[code] = getattr(self.ff, f"calc_{stock_factor}")(
                    ts_code=code, start_date=start_date, end_date=date
                )
            except Exception:
                continue
        ...
        processed_stock_factor = self.fp.process_factor(
            stock_factor_series, neutralize=False
        )  # 在行业内选股，通常不进行行业中性化

        final_portfolio = pd.Series(dtype=float)
        df_for_selection = pd.DataFrame({"factor": processed_stock_factor}).merge(
            self.ia.stock_basics[["ts_code", "industry"]],
            left_index=True,
            right_on="ts_code",
        )
        ...
        for industry in top_industries.index:
            ...
            top_stocks = industry_stocks.sort_values("factor", ascending=False).head(
                top_n_stocks_per_industry
            )
            final_portfolio = pd.concat(
                [final_portfolio, top_stocks.set_index("ts_code")["factor"]]
            )
...
class HistoricalScorer:
    ...
    def __init__(
        self, data_manager: data.DataManager, factor_processor: FactorProcessor
    ):
    ...
    def calculate_fund_score_series(
        self, ts_code: str, start_date: str, end_date: str
    ) -> pd.Series:
        """计算单只股票的历史资金得分序列"""
        query = f"""
        ...
        # 归一化到 0-100
        normalized_score = (
            (smoothed_score - smoothed_score.min())
            / (smoothed_score.max() - smoothed_score.min())
            * 100
        )
    ...
    def calculate_macro_score_series(self, start_date: str, end_date: str) -> pd.Series:
        ...
        df_macro = pd.merge(
            df_pmi[["month", "pmi010000"]],
            df_m[["month", "m1_yoy", "m2_yoy"]],
            on="month",
        )
        ...
        m_gap_z = (df_macro["m1_m2_gap"] - df_macro["m1_m2_gap"].mean()) / df_macro[
            "m1_m2_gap"
        ].std()
        ...
        # 归一化到 0-100
        normalized_score = (
            (macro_score - macro_score.min())
            / (macro_score.max() - macro_score.min())
            * 100
        )
...
class LSTMAlphaStrategy:
    ...
    def __init__(
        self, data_manager: data.DataManager, lookback_window=60, prediction_horizon=5
    ):
        self.dm = data_manager
...
if __name__ == "__main__":
    ...
    log.info("\n【功能3】演示分层回测...")
    layered_results, fig = analyzer.run_layered_backtest(
        loaded_factor, num_quantiles=3, forward_return_period=20
    )
    ...
    mock_cov = pd.DataFrame(
        np.random.rand(5, 5) / 100, index=tickers, columns=tickers
    ).abs()
    mock_cov = mock_cov + mock_cov.T / 2
    np.fill_diagonal(mock_cov.values, np.random.rand(5) / 20)

    mock_industry_map = pd.DataFrame(
        {"ts_code": tickers, "industry": ["科技", "科技", "科技", "零售", "汽车"]}
    )

    optimizer = AdvancedPortfolioOptimizer(mock_returns, mock_cov, mock_industry_map)
    optimized_w = optimizer.optimize(
        max_weight_per_stock=0.3, industry_max_exposure=0.6
    )

    log.info("优化后的权重:\n%s", optimized_w)
    if optimized_w is not None:
        tech_sum = (
            optimized_w.join(mock_industry_map.set_index("ts_code"))
