...
def run_daily_strategy_workflow():
    ...
    try:
        # a. 数据准备
        stock_pool = dm.get_stock_basic()["ts_code"].tolist()[
            :300
        ]  # 为提高演示速度，缩小范围
        prices_start_date = (
            pd.to_datetime(trade_date)
            - timedelta(days=strategy_config["params"]["ic_lookback_days"] + 60)
        ).strftime("%Y%m%d")
        ...
        # c. 优化得到最终持仓
        selected_stocks = composite_factor.nlargest(
            strategy_config["params"]["top_n_stocks"]
        ).index
        cov_matrix = (
        ...
        target_portfolio = optimizer.optimize_max_sharpe(
            max_weight_per_stock=strategy_config["params"]["max_weight_per_stock"]
        )
    ...
    try:
        risk_exposure = risk_manager.calculate_risk_exposure(
            target_portfolio["weight"], trade_date
        )
        log.info(f"风险分析完毕。")
    except Exception as e:
...
def generate_pre_market_plan(
    target_portfolio: pd.DataFrame, stock_basics: pd.DataFrame
) -> dict:
    """
    ...
    # 合并股票名称和行业信息
    portfolio_df = target_portfolio.merge(
        stock_basics[["ts_code", "name", "industry"]], on="ts_code"
    )
