...
def initialize_all_data():
    ...
    try:
        data_manager = data.DataManager(token=config.TUSHARE_TOKEN, db_url=config.DATABASE_URL)
        ...
        start_date = (datetime.now() - timedelta(days=10 * 365)).strftime('%Y%m%d') # 拉取10年数据
        ...
        for asset_type, codes in core_assets.items():
            for code in codes:
                ...
                if asset_type == 'indices':
                    # 对指数，调用专用的指数日线接口
                    df = data_manager.get_index_daily(ts_code=code, start_date=start_date, end_date=end_date)
                elif asset_type == 'stocks':
                    # 对股票，调用复权行情接口
                    df = data_manager.get_adjusted_daily(ts_code=code, start_date=start_date, end_date=end_date)
