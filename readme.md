...
# 方法3: 运行单个测试
python run_system_check.py          # 统一系统健康检查
python run_system_check.py --mode quick      # 快速检查
python run_system_check.py --mode stability  # 稳定性检查
```

### 📋 测试说明

- **test_core_functions.py**: 测试最基础的连接、获取、计算、存储功能
- **run_system_check.py**: 统一的系统健康检查工具，替代所有旧的测试脚本
  - 支持快速检查、完整检查和稳定性检查模式
  - 集成了配置验证、数据库检查、API连通性、因子计算等功能
