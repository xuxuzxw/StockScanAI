# StockScanAI/industry_analyzer.py
import pandas as pd
...
class IndustryAnalyzer:
    ...
    def __init__(
        self, data_manager: data.DataManager, factor_factory: qe.FactorFactory
    ):
    ...
    def get_industry_factor_rank(
        self, date: str, factor_name: str, ascending: bool = False
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            merged_df = pd.merge(
                factor_df, self.stock_basics[["ts_code", "industry"]], on="ts_code"
            )

            # 4. 分组、聚合、排序
            industry_rank = (
                merged_df.groupby("industry")["factor_value"]
                .mean()
