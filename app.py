...
COLUMN_MAPPING = {
    ...
    # 财务类 - 业绩预告/快报
    "type": "预告类型",
    "p_change_min": "业绩变动(最小%)",
    "p_change_max": "业绩变动(最大%)",
    "net_profit_min": "净利润(最小)",
    "net_profit_max": "净利润(最大)",
    ...
    "invest_income": "投资收益",
    "ass_invest_income": "对联营和合营企业的投资收益",
    "forex_gain": "汇兑收益",
    "total_cogs": "营业总成本",
    "oper_cost": "营业成本",
    "int_exp": "利息支出",
    ...
    "compr_inc_attr_p": "归母综合收益总额",
    "compr_inc_attr_m_s": "归属少数股东综合收益总额",
    "ebit": "息税前利润",
    "ebitda": "息税折旧摊销前利润",
    "insurance_exp": "保险合同准备金",
    "undist_profit": "未分配利润",
    ...
    "lt_borr": "长期借款",
    "cb_borr": "向中央银行借款",
    "depos_ib_deposits": "同业及其他金融机构存放款项",
    "loan_oth_bank": "拆入资金",
    "trading_fl": "交易性金融负债",
    "notes_payable": "应付票据",
    "acct_payable": "应付账款",
    ...
    "defer_inc_non_cur_liab": "非流动负债:递延收益",
    "oth_ncl": "其他非流动负债",
    "total_ncl": "非流动负债合计",
    ...
    "hfs_assets": "持有待售的资产",
    "hfs_sales": "持有待售的负债",
    "cost_fin_assets": "融出资金",
    "fair_value_fin_assets": "以公允价值计量的金融资产",
...
# --- 页面基础设置 ---
st.set_page_config(
    page_title="AI量化投研平台 V3 - 全功能版", page_icon="🏆", layout="wide"
)
...
def initialize_components():
    ...
    try:
        ...
        ai_orchestrator = intelligence.AIOrchestrator(
            config.AI_MODEL_CONFIG, data_manager
        )
    ...
    all_prices_df = pd.DataFrame(
        {
            ...
            for stock, df in prices_dict.items()
...
if stock_list is not None and not stock_list.empty:
    ...
    if (
        "selected_stock" in st.session_state
        and st.session_state.selected_stock
        and st.session_state.selected_stock in stock_options.tolist()
    ):
        default_index = stock_options.tolist().index(st.session_state.selected_stock)
    else:  # 否则使用默认值
        default_index = (
            stock_options[stock_options.str.contains("贵州茅台")].index[0]
            if any(stock_options.str.contains("贵州茅台"))
            else 0
        )
...
if stock_list is not None and not stock_list[stock_list["ts_code"] == ts_code].empty:
    ...
    st.markdown(
        f"**名称:** {info['name']} | **行业:** {info['industry']} | **上市:** {info['list_date']}"
    )
...
tab_mapping = {
    ...
    .replace("🚀_", "")
    .replace("⚙️_", ""): tab_obj
    for tab, tab_obj in zip(tab_list, tabs)
}
...
if tab_decision:
    with tab_decision:
        ...
        st.markdown(
            "基于每日自动化策略 (`run_strategy_daily.py`) 的执行结果，生成高保真实战决策支持信息。"
        )

        # 从策略看板获取最新交易日
        try:
            cal_df = data_manager.pro.trade_cal(
                exchange="",
            ...
            with st.spinner("正在加载预计算的交易计划..."):
                try:
                    ...
                    if os.path.exists(plan_path):
                        ...
                        st.error(
                            f"错误：未找到 {report_date} 的交易计划文件。请确认后台 `run_strategy_daily.py` 任务是否已成功执行。"
                        )
        ...
        st.info(
            "此模块为V2.4规划功能，将在未来版本中实现。它将分析当日实际成交回报与预期的偏差，并对交易成本、冲击成本等进行量化归因。"
        )
        # 预留的UI布局
...
if tab_strategy_board:
    with tab_strategy_board:
        ...
        st.markdown(
            "此看板展示由后台自动化工作流在每日开盘前（默认 08:00）生成的最新策略分析结果。"
        )
        ...
        try:
            with data_manager.engine.connect() as conn:
                query = text(
                    "SELECT report_content FROM ai_reports WHERE trade_date = :date AND ts_code = 'STRATEGY_MORNING_REPORT'"
                )
                report_content = conn.execute(
                    query, {"date": report_date}
                ).scalar_one_or_none()
            ...
            if report_content:
                ...
                st.warning(
                    f"未能在数据库中找到 {report_date} 的晨报。请确认后台 `run_strategy_daily.py` 任务是否已成功执行。"
                )
        except Exception as e:
...
if tab_ranker:
    with tab_ranker:
        ...
        st.markdown(
            "构建您的专属多因子模型，系统将从**行业**和**个股**两个层面进行综合打分排名，助您实现“先选赛道、再选赛马”的专业投研。"
        )
        # --- 1. 获取最新交易日 ---
        try:
            cal_df = data_manager.pro.trade_cal(
        ...
        if latest_trade_date:
            ...
            QUALITY_GROWTH_FACTORS = [
                "roe",
                "growth_revenue_yoy",
                "debt_to_assets",
                "forecast_growth_rate",
            ]
            TECH_FINANCE_FACTORS = [
                "momentum",
                "volatility",
                "net_inflow_ratio",
                "block_trade_ratio",
            ...
            with cols[0]:
                st.multiselect(
                    "价值/回报因子",
                    [f for f in available_factors if f in VALUE_FACTORS],
                    default=["pe_ttm", "dividend_yield", "repurchase_ratio"],
                    key="value_factors",
                )
            with cols[1]:
                st.multiselect(
                    "质量/成长因子",
                    [f for f in available_factors if f in QUALITY_GROWTH_FACTORS],
                    default=["roe", "growth_revenue_yoy", "forecast_growth_rate"],
                    key="quality_factors",
                )
            with cols[2]:
                st.multiselect(
                    "技术/资金因子",
                    [f for f in available_factors if f in TECH_FINANCE_FACTORS],
                    default=["momentum", "net_inflow_ratio"],
                    key="tech_factors",
                )
            with cols[3]:
                st.multiselect(
                    "筹码因子",
                    [f for f in available_factors if f in CHIP_FACTORS],
                    default=[
                        "holder_num_change_ratio",
                        "major_shareholder_net_buy_ratio",
                    ],
            ...
            user_selection = (
                ...
                + st.session_state.quality_factors
                + st.session_state.tech_factors
                + st.session_state.chip_factors
            )
            # --- 3. 执行排名 ---
                ...
                if not user_selection:
                    ...
                    with st.spinner("正在从因子库提取数据并计算行业与个股综合得分..."):
                        try:
                            ...
                            # --- B. 数据处理：将长表转换为宽表 ---
                            factor_table = all_factor_data.pivot(
                                index="ts_code",
                                columns="factor_name",
                                values="factor_value",
                            ).dropna()
                            ...
                            factor_table_with_industry = factor_table.merge(
                                full_stock_list[["ts_code", "name", "industry"]],
                                on="ts_code",
                            )
                            ...
                            industry_factors = factor_table_with_industry.groupby(
                                "industry"
                            )[user_selection].mean()
                            processed_industry_factors = industry_factors.apply(
                            ...
                            processed_industry_factors["行业综合得分"] = (
                                processed_industry_factors.mean(axis=1)
                            )
                            industry_rank = processed_industry_factors.sort_values(
                                "行业综合得分", ascending=False
                            )
                            ...
                            processed_stock_factors = factor_table.apply(
                                lambda x: (x - x.mean()) / x.std()
                            )
                            for factor, direction in factor_direction.items():
                                if factor in processed_stock_factors.columns:
                                    processed_stock_factors[factor] *= direction
                            processed_stock_factors["综合得分"] = (
                                processed_stock_factors.mean(axis=1)
                            )

                            final_rank = processed_stock_factors.merge(
                                full_stock_list[["ts_code", "name", "industry"]],
                                on="ts_code",
                            )
                            ...
                            # --- F. 个股结果展示与交互 ---
                            final_rank_display = final_rank[
                                ["ts_code", "name", "industry", "综合得分"]
                            ].head(100)

                            st.caption(
                                "💡 小提示：直接点击下方表格中的任意一行，系统将自动跳转到该股票的深度分析页面。"
                            )
                            ...
                            # 检查是否有行被选中
                            if (
                                st.session_state.rank_selector
                                and st.session_state.rank_selector.get(
                                    "selection", {}
                                ).get("rows")
                            ):
                                selected_index = st.session_state.rank_selector[
                                    "selection"
                                ]["rows"][0]
                                ...
                                st.session_state.selected_stock = (
                                    f"{selected_ts_code} {selected_name}"
                                )
                                st.rerun()
...
if tab_main:
    with tab_main:
        ...
        df_adj = data_manager.get_adjusted_daily(
            ts_code, start_date_str, end_date_str, adj="hfq"
        )
        if df_adj is not None and not df_adj.empty:
            # --- 1. 数据获取与合并 ---
            # 获取每日基本面指标（PE、换手率等）
            df_basic = data_manager.get_daily_basic(
                ts_code, start_date_str, end_date_str
            )
            if df_basic is not None and not df_basic.empty:
                ...
                df_adj = pd.merge(
                    df_adj,
                    df_basic[["trade_date", "pe_ttm", "turnover_rate"]],
                    on="trade_date",
                    how="left",
                )
...
if tab_funds:
    with tab_funds:
        ...
        with col1:
            ...
            df_flow = data_manager.get_moneyflow(
                ts_code,
        ...
        with col3:
            ...
            if (
                df_holder_num is not None
                and not df_holder_num.empty
                and len(df_holder_num) > 1
            ):
                df_holder_num["end_date"] = pd.to_datetime(df_holder_num["end_date"])
                fig = go.Figure()
                fig.add_trace(
                    go.Scatter(
                        x=df_holder_num["end_date"],
                        y=df_holder_num["holder_num"],
                        mode="lines+markers",
                        name="股东人数",
                    )
                )
        ...
        with col4:
            ...
            df_top_list_hist = data_manager.get_top_list(
                start_date=(end_date - timedelta(days=90)).strftime("%Y%m%d"),
                end_date=end_date_str,
            )
            if df_top_list_hist is not None and not df_top_list_hist.empty:
                df_top_list_hist["trade_date"] = pd.to_datetime(
                    df_top_list_hist["trade_date"]
                )
                stock_top_list = df_top_list_hist[
                    df_top_list_hist["ts_code"] == ts_code
                ]
                if not stock_top_list.empty:
                    fig = go.Figure()
                    fig.add_trace(
                        go.Bar(
                            x=stock_top_list["trade_date"],
                            y=stock_top_list["net_amount"],
                            name="龙虎榜净买入额",
                        )
                    )
                    fig.update_layout(
        ...
        df_holder_trade = data_manager.get_holder_trade(
            ts_code,
        ...
        if df_holder_trade is not None and not df_holder_trade.empty:
            ...
            df_display = df_display[
                ["ann_date", "holder_name", "in_de", "change_ratio", "avg_price"]
        ...
        df_block_trade = data_manager.get_block_trade(
            start_date=(end_date - timedelta(days=90)).strftime("%Y%m%d"),
            end_date=end_date_str,
        )
        if df_block_trade is not None and not df_block_trade.empty:
            ...
            if not stock_block_trade.empty:
                df_display = stock_block_trade.sort_values(
        ...
        if latest_period:
            ...
            st.dataframe(
                df_display, use_container_width=True, height=385, hide_index=True
            )
        else:
    ...
    with tab_finance:
        ...
        with st.spinner("正在计算历史财务得分..."):
            ...
            if not financial_score_series.empty:
                ...
                st.caption(
                    "得分基于ROE、营收增长、净利增长和资产负债率等指标的历史百分位排名等权合成。"
                )
            else:
        ...
        def display_transposed_df(df: pd.DataFrame):
            ...
            df_display["指标"] = (
                df_display["指标"].map(COLUMN_MAPPING).fillna(df_display["指标"])
            )
            df_display["数值"] = df_display["数值"].astype(str)
            st.dataframe(df_display, use_container_width=True, hide_index=True)

        df_express = data_manager.get_express(
            ts_code,
            start_date=(end_date - timedelta(days=365)).strftime("%Y%m%d"),
            end_date=end_date_str,
        )
        if df_express is not None and not df_express.empty:
            ...
            latest_pit_express = (
                df_express[df_express["ann_date"] <= end_date]
                .sort_values(by="ann_date", ascending=False)
                .head(1)
            )
        ...
        df_forecast = data_manager.get_forecast(
            ts_code,
            start_date=(end_date - timedelta(days=365)).strftime("%Y%m%d"),
            end_date=end_date_str,
        )
        if df_forecast is not None and not df_forecast.empty:
            ...
            latest_pit_forecast = (
                df_forecast[df_forecast["ann_date"] <= end_date]
                .sort_values(by="ann_date", ascending=False)
                .head(1)
            )
        ...
        if df_dividend is not None and not df_dividend.empty:
            ...
            df_display = df_display[
                ["end_date", "ann_date", "div_proc", "stk_div", "cash_div_tax"]
            ].rename(columns=COLUMN_MAPPING)
        ...
        df_repurchase = data_manager.get_repurchase(
            ts_code,
            start_date=(end_date - timedelta(days=365)).strftime("%Y%m%d"),
            end_date=end_date_str,
        )
        if df_repurchase is not None and not df_repurchase.empty:
            ...
            df_display = df_display[
                ["ann_date", "proc", "vol", "amount", "high_limit", "low_limit"]
            ].rename(columns=COLUMN_MAPPING)
            st.dataframe(df_display, use_container_width=True, hide_index=True)
        else:
            st.info("近一年无股票回购记录。")
...
if tab_macro:
    with tab_macro:
        ...
        with st.spinner("正在计算历史宏观景气得分..."):
            scorer = quant_engine.HistoricalScorer(data_manager, factor_processor)
            macro_score_series = scorer.calculate_macro_score_series(
        ...
        if df_pmi is not None and not df_pmi.empty:
            ...
            if date_col not in df_pmi.columns or pmi_col not in df_pmi.columns:
                st.error(
                    f"PMI数据中未找到关键列。需要日期列 ('{date_col}') 和PMI列 ('{pmi_col}')。可用列：{df_pmi.columns.tolist()}"
                )
            else:
                fig.add_trace(
                    go.Scatter(
                        x=df_pmi[date_col],
                        y=pd.to_numeric(df_pmi[pmi_col]),
                        name="制造业PMI",
                    )
                )
                fig.add_hline(
                ...
                fig.update_layout(
                    title="制造业采购经理人指数 (PMI)", template="plotly_dark"
                )
                st.plotly_chart(fig, use_container_width=True)
        ...
        if df_m is not None and not df_m.empty:
            fig = go.Figure()
            date_col = (
                "month" if "month" in df_m.columns else "stat_month"
            )  # 兼容'month'或'stat_month'
            ...
            if date_col not in df_m.columns:
                ...
                fig.add_trace(
                    go.Bar(
                        x=df_m[date_col],
                        y=df_m["m1_yoy"] - df_m["m2_yoy"],
                        name="M1-M2剪刀差",
                    )
                )
                fig.add_hline(y=0, line_dash="dash", line_color="white")
                fig.update_layout(
                    title="M1-M2同比增速剪刀差(%)", template="plotly_dark"
                )
                st.plotly_chart(fig, use_container_width=True)
        ...
        if df_cpi is not None and not df_cpi.empty:
            fig = go.Figure()
            fig.add_trace(
                go.Bar(x=df_cpi["month"], y=df_cpi["nt_yoy"], name="CPI全国同比(%)")
            )
            fig.update_layout(title="居民消费价格指数 (CPI)", template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)

        df_shibor = data_manager.get_shibor(
            start_date=(end_date - timedelta(days=365)).strftime("%Y%m%d"),
            end_date=end_date_str,
        )
        if df_shibor is not None and not df_shibor.empty:
            ...
            fig.update_layout(
                title="上海银行间同业拆放利率 (Shibor)", template="plotly_dark"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        st.caption(
            "基于PMI和M1-M2剪刀差的宏观模型。策略应根据'牛市'、'熊市'或'震荡市'调整风险偏好。"
        )
...
if V2_MODULES_LOADED and tab_market:
    with tab_market:
        ...
        with st.spinner("正在计算大盘估值..."):
            valuation = index_analyzer_client.get_index_valuation_percentile(
                index_code, end_date_str
            )
            timing_signal = index_analyzer_client.generate_timing_signal(
                index_code, end_date_str
            )
            ...
            if "error" in valuation:
                ...
                col1.metric(
                    label="PE(TTM) 百分位",
                    value=f"{valuation['pe_percentile']:.2%}",
                    help=f"当前PE: {valuation['pe_ttm']:.2f}。百分位越低代表估值越便宜。",
                )
                col2.metric(
                    label="PB 百分位",
                    value=f"{valuation['pb_percentile']:.2%}",
                    help=f"当前PB: {valuation['pb']:.2f}。百分位越低代表估值越便宜。",
                )
                st.info(
                    "择时信号基于PE和PB在过去5年的历史百分位生成，可用于辅助判断市场整体风险。"
                )
    ...
    with tab_industry:
        ...
        if not industry_map:
            ...
            selected_industry = st.selectbox(
                "选择要分析的行业板块:", options=list(industry_map.keys())
            )
            industry_index_code = industry_map.get(selected_industry)

            if industry_index_code:
                with st.spinner(
                    f"正在加载 {selected_industry} ({industry_index_code}) 的行情数据..."
                ):
                    # 调用 get_index_daily 获取行业指数数据
                    df_industry = data_manager.get_index_daily(
                        industry_index_code, start_date_str, end_date_str
                    )

                    if df_industry is not None and not df_industry.empty:
                        # --- 复用个股行情的K线绘图逻辑 ---
                        df_industry["EMA20"] = (
                            df_industry["close"].ewm(span=20, adjust=False).mean()
                        )
                        df_industry["EMA60"] = (
                            df_industry["close"].ewm(span=60, adjust=False).mean()
                        )
                        df_industry["Vol_EMA20"] = (
                            df_industry["vol"].ewm(span=20, adjust=False).mean()
                        )
                        ...
                        fig.add_trace(
                            go.Candlestick(
                                x=df_industry["trade_date"],
                                open=df_industry["open"],
                                high=df_industry["high"],
                                low=df_industry["low"],
                                close=df_industry["close"],
                                name="行业指数K线",
                            ),
                            row=1,
                            col=1,
        ...
        st.markdown(
            "计算全市场所有申万一级行业的平均因子暴露值，并进行排序，用于发现强势或弱势行业。"
        )

        # 1. 初始化行业分析器
        try:
            industry_analyzer_client = industry_analyzer.IndustryAnalyzer(
                data_manager, factor_factory
            )
            ANALYZER_READY = True
        except Exception as e:
            ...
            if st.button("开始行业排名分析"):
                with st.spinner(f"正在从因子库查询 '{factor_to_rank}' 的行业排名..."):
                    try:
                        ...
                        if ranked_df.empty:
                            st.warning(
                                f"在因子库中未找到 {end_date_str} 的 {factor_to_rank} 数据。请确认后台计算任务是否已成功执行。"
                            )
                        else:
                            st.success("行业排名查询完成！")
                            ...
                            st.markdown(
                                f"#### **{factor_to_rank}** 因子排名前10行业可视化"
                            )
                            top_10_df = (
                                ranked_df.head(10)
                                if not ascending
                                else ranked_df.tail(10).sort_values(
                                    by="factor_value", ascending=False
                                )
                            )
...
if tab_ai:
    with tab_ai:
        ...
        st.markdown(
            "点击下方按钮，AI将采集并分析该股的 **技术、资金、财务、筹码、宏观、舆情** 六大维度数据，生成一份深度综合投研报告。"
        )
        # FIX 1: Corrected indentation for the 'with' block under the 'if' statement.
        if st.button(
            "🚀 启动AI深度综合分析",
            help="调用混合AI引擎，对该股票进行六大维度、递进式分析，生成综合投研报告。",
        ):
            with st.spinner("AI引擎启动...正在执行多维数据采集与深度分析工作流..."):
                try:
                    ...
                    st.markdown(report)
                    st.info(
                        f"本次分析调用AI模型 {cost['total_calls']} 次, 预估成本: ${cost['estimated_cost']:.4f}。"
                    )
                except Exception as e:
...
if tab_analyzer:
    with tab_analyzer:
        ...
        st.markdown(
            "选择一个或多个因子，在指定的股票池和时间段内，进行IC/IR分析和分层回测，以评估其选股有效性。"
        )
        # --- 参数配置 ---
        ...
        with analyzer_cols[1]:
            analyzer_start_date = st.date_input(
                "分析开始日期", datetime(2023, 1, 1), key="analyzer_start"
            )
        with analyzer_cols[2]:
            analyzer_end_date = st.date_input(
                "分析结束日期", datetime.now() - timedelta(days=1), key="analyzer_end"
            )

        analyzer_stock_pool_options = (
            get_stock_list()["ts_code"] + " " + get_stock_list()["name"]
        )
        # 默认选择一个包含不同行业的股票池作为示例
        default_analyzer_pool = [
            s
            for s in analyzer_stock_pool_options
            if any(k in s for k in ["平安", "茅台", "宁德", "万科", "中信"])
        ]
            ...
            if not analyzer_stock_codes:
                ...
                with st.spinner(f"正在分析因子 '{factor_to_analyze}'..."):
                    try:
                        ...
                        dates = pd.date_range(
                            analyzer_start_date, analyzer_end_date, freq="M"
                        )
                        ...
                        for i, date in enumerate(dates):
                            ...
                            for code in analyzer_stock_codes:
                                calc_func = getattr(
                                    factor_factory, f"calc_{factor_to_analyze}"
                                )
                                # 因子函数需要不同的参数，这里做一个适配
                                if factor_to_analyze in [
                                    "momentum",
                                    "volatility",
                                    "net_inflow_ratio",
                                    "north_hold_change",
                                ]:
                                    raw_values[code] = calc_func(
                                        ts_code=code,
                                        start_date=start_date_str_calc,
                                        end_date=date_str,
                                    )
                                else:  # 基本面等因子
                                    raw_values[code] = calc_func(
                                        ts_code=code, date=date_str
                                    )
                        ...
                        for date, factor_slice in factor_df.iterrows():
                            ic, p_val = factor_analyzer.calculate_ic(
                                factor_slice.dropna()
                            )
                            if not np.isnan(ic):
                                ic_values[date] = ic
                        ...
                        layered_results, fig = factor_analyzer.run_layered_backtest(
                            factor_df, num_quantiles=5, forward_return_period=20
                        )
                        st.success("分层回测完成！")
...
if tab_backtest:
    with tab_backtest:
        ...
        backtest_type = st.radio(
            "选择回测类型:",
            ("向量化回测 (速度快，适合多因子)", "事件驱动回测 (精度高，模拟真实交易)"),
        )

        if backtest_type == "向量化回测 (速度快，适合多因子)":
            st.markdown("---")
            st.markdown(
                "构建多因子策略，通过投资组合优化器生成权重，并在考虑交易成本和风控规则下进行回测。"
            )
            ...
            if strategy_model == "固定权重多因子":
                ...
                factor_weights = {}
                factor_weights["momentum"] = st.slider(
                    "动量因子 (Momentum) 权重:", -1.0, 1.0, 0.5, 0.1
                )
                factor_weights["volatility"] = st.slider(
                    "低波动因子 (Volatility) 权重:", -1.0, 1.0, -0.3, 0.1
                )  # 权重为负代表选取波动率低的
                factor_weights["net_inflow_ratio"] = st.slider(
                    "资金流入因子 (Net Inflow) 权重:", -1.0, 1.0, 0.2, 0.1
                )
            ...
            with col1:
                bt_start_date = st.date_input(
                    "回测开始日期", datetime(2023, 1, 1), key="bt_start"
                )
            with col2:
                bt_end_date = st.date_input(
                    "回测结束日期", datetime.now() - timedelta(days=1), key="bt_end"
                )
            with col3:
                rebalance_freq = st.selectbox(
                    "调仓频率", ["M", "W"], index=0, help="M=月度调仓, W=周度调仓"
                )
            ...
            with col5:
                max_weight = (
                    st.number_input("单票最大权重(%)", 1.0, 100.0, 10.0, 1.0) / 100
                )
            with col6:
                stop_loss = (
                    st.number_input(
                        "止损线(%)", 0.0, 50.0, 15.0, 1.0, help="0表示不止损"
                    )
                    / 100
                )
                ...
                with st.spinner("正在执行向量化回测..."):
                    try:
                        ...
                        if strategy_model == "自适应权重 (IC-IR)":
                            prices_start_str = (
                                bt_start_date - timedelta(days=ic_lookback_days + 60)
                            ).strftime("%Y%m%d")

                        prices_dict = data_manager.run_batch_download(
                            stock_pool, prices_start_str, bt_end_str
                        )
                        ...
                        rebalance_dates = rebalance_dates[
                            (rebalance_dates >= backtest_prices.index.min())
                            & (rebalance_dates <= backtest_prices.index.max())
                        ]
                        ...
                        all_weights_df = pd.DataFrame(
                            index=backtest_prices.index, columns=stock_pool
                        )
                        progress_bar = st.progress(0)
                        optimized_weights = (
                            pd.DataFrame()
                        ...
                        if strategy_model == "固定权重多因子":
                            for i, date in enumerate(rebalance_dates):
                                ...
                                factor_start_str = (date - timedelta(days=60)).strftime(
                                    "%Y%m%d"
                                )
                                for factor_name, weight in factor_weights.items():
                                    if weight == 0:
                                        continue
                                    raw_values = {
                                        s: factor_factory.calculate(
                                            factor_name,
                                            ts_code=s,
                                            start_date=factor_start_str,
                                            end_date=factor_date_str,
                                        )
                                        for s in stock_pool
                                    }
                                    ...
                                    if raw_series.empty:
                                        continue
                                    processed_factor = factor_processor.process_factor(
                                        raw_series, neutralize=True
                                    )
                                    if composite_factor.empty:
                                        composite_factor = (
                                            processed_factor.mul(weight)
                                            .reindex(stock_pool)
                                            .fillna(0)
                                        )
                                    else:
                                        composite_factor = composite_factor.add(
                                            processed_factor.mul(weight), fill_value=0
                                        )

                                if (
                                    composite_factor.empty
                                    or composite_factor.sum() == 0
                                ):
                                    continue
                                selected_stocks = composite_factor.nlargest(20).index
                                cov_matrix = (
                                    all_prices_df[selected_stocks]
                                    .loc[:date]
                                    .pct_change()
                                    .iloc[-252:]
                                    .cov()
                                ...
                                optimizer = quant_engine.PortfolioOptimizer(
                                    expected_returns, cov_matrix
                                )
                                optimized_weights = optimizer.optimize_max_sharpe(
                        ...
                        # --- B. 自适应权重策略 ---
                            ...
                            adaptive_strategy = quant_engine.AdaptiveAlphaStrategy(
                                factor_factory,
                                factor_processor,
                                factor_analyzer,
                                all_prices_df,
                            ...
                            for i, date in enumerate(rebalance_dates):
                                composite_factor, dynamic_weights = (
                                    adaptive_strategy.generate_composite_factor(
                                        date,
                                        stock_pool,
                                        tuple(factors_to_use_adaptive),
                                        ic_lookback_days,
                                    )
                                )
                                if i == 0:
                        ...
                        # --- C. 机器学习策略 ---
                            ...
                            model_loaded = (
                                ml_strategy.load_model()
                            )  # 默认加载 ml_model.pkl

                            if not model_loaded:
                                st.error(
                                    "错误：找不到已训练的模型文件 (ml_model.pkl)。请先在“模型训练室”中训练并保存模型。"
                                )
                                st.stop()
                            st.success("  模型加载成功！")
                            ...
                            for i, date in enumerate(rebalance_dates):
                                ...
                                selected_stocks = ml_strategy.predict_top_stocks(
                                    factor_snapshot, top_n=20
                                )

                                if len(selected_stocks) > 0:
                                    weights = 1.0 / len(selected_stocks)
                                    optimized_weights = pd.DataFrame(
                                        {"weight": weights}, index=selected_stocks
                                    )
                                    all_weights_df.loc[date] = optimized_weights[
                                        "weight"
                                    ]
                        ...
                        all_weights_df = (
                            all_weights_df.reindex(backtest_prices.index)
                            .ffill()
                            .fillna(0)
                        )
                        st.success("权重填充完毕！")
                        ...
                        if not optimized_weights.empty:
                            ...
                            st.dataframe(
                                optimized_weights.style.format({"weight": "{:.2%}"})
                            )
                        st.markdown("#### 净值曲线与回撤")
                        ...
                        with st.spinner("正在执行风险暴露分析..."):
                            try:
                                risk_manager = quant_engine.RiskManager(
                                    factor_factory, factor_processor
                                )
                                valid_rebalance_dates = all_weights_df[
                                    all_weights_df.sum(axis=1) > 0
                                ].index
                                all_exposures = []
                                for date in valid_rebalance_dates:
                                    portfolio_weights = all_weights_df.loc[
                                        date
                                    ].dropna()
                                    ...
                                    if not portfolio_weights.empty:
                                        exposure = risk_manager.calculate_risk_exposure(
                                            portfolio_weights, date.strftime("%Y%m%d")
                                        )
                                        exposure.name = date
                                ...
                                if all_exposures:
                                    ...
                                    st.plotly_chart(fig, use_container_width=True)
                                    st.caption(
                                        "暴露值为正，表示您的投资组合在该风险因子上呈正向暴露；值为负则相反。接近0表示在该风险上表现中性。"
                                    )
                                else:
                        ...
                        with st.spinner("正在执行Brinson归因分析..."):
                            try:
                                ...
                                if (
                                    len(rebalance_dates_attr) > 1
                                    and not optimized_weights.empty
                                ):
                                    attribution_period_start = rebalance_dates_attr[0]
                                    attribution_period_end = rebalance_dates_attr[-1]
                                    stock_basics = get_stock_list()
                                    stock_industry_map = stock_basics[
                                        stock_basics["ts_code"].isin(stock_pool)
                                    ][["ts_code", "industry"]]
                                    portfolio_weights_for_attr = optimized_weights[
                                        "weight"
                                    ]
                                    benchmark_weights_for_attr = pd.Series(
                                        1 / len(stock_pool), index=stock_pool
                                    )
                                    period_returns = (
                                        all_prices_df.loc[attribution_period_end]
                                        / all_prices_df.loc[attribution_period_start]
                                        - 1
                                    )

                                    attribution_analyzer = quant_engine.PerformanceAttribution(
                                        portfolio_returns=period_returns.reindex(
                                            portfolio_weights_for_attr.index
                                        ).fillna(0),
                                        portfolio_weights=portfolio_weights_for_attr,
                                        benchmark_returns=period_returns.reindex(
                                            benchmark_weights_for_attr.index
                                        ).fillna(0),
                                        benchmark_weights=benchmark_weights_for_attr,
                                        stock_industry_map=stock_industry_map,
                                    )
                                    attribution_results = (
                                        attribution_analyzer.run_brinson_attribution()
                                    )
                                    st.dataframe(attribution_results)
                                    st.caption(
                                        "正向的'资产配置'表示策略超配了表现优于基准的行业。正向的'个股选择'表示在行业内部选出的个股表现优于该行业的整体基准。"
                                    )
                                else:
            ...
            st.markdown("---")
            st.markdown(
                "模拟真实的逐日交易过程，策略在每个交易日接收数据并做出决策，适合验证均线、突破等时序型策略。"
            )

            st.markdown("#### 1. 配置回测参数")
            ed_col1, ed_col2, ed_col3 = st.columns(3)
            with ed_col1:
                ed_start_date = st.date_input(
                    "回测开始日期", datetime(2023, 1, 1), key="ed_start"
                )
            with ed_col2:
                ed_end_date = st.date_input(
                    "回测结束日期", datetime.now() - timedelta(days=1), key="ed_end"
                )
            with ed_col3:
                initial_capital = st.number_input(
                    "初始资金", 100000, 100000000, 1000000, 100000
                )
            ...
            ed_stock_pool = st.multiselect(
                "选择股票池 (建议3-5支)",
                options=stock_pool_options,
                default=[s for s in stock_pool_options if "茅台" in s or "平安" in s],
            )
                ...
                if not ed_stock_codes:
                    ...
                    with st.spinner("正在执行事件驱动回测，请稍候..."):
                        try:
                            ...
                            prices_dict = data_manager.run_batch_download(
                                ed_stock_codes, ed_start_str, ed_end_str
                            )
                            ...
                            all_volumes_df = pd.DataFrame(
                                {
                                    ...
                                    if df is not None
                                    and not df.empty
                                    and "vol" in df.columns
                                }
                            ).sort_index()

                            common_index = all_prices_df.index.intersection(
                                all_volumes_df.index
                            )
                            common_columns = all_prices_df.columns.intersection(
                                all_volumes_df.columns
                            )
                            all_prices_df = all_prices_df.loc[
                                common_index, common_columns
                            ]
                            all_volumes_df = all_volumes_df.loc[
                                common_index, common_columns
                            ]
                            all_prices_df.dropna(axis=1, how="all", inplace=True)
                            all_volumes_df = all_volumes_df.reindex(
                                columns=all_prices_df.columns
                            )

                            st.success(
                                f"价格与成交量数据准备完成！股票池: {all_prices_df.columns.tolist()}"
                            )
                            ...
                            data_handler = quant_engine.SimpleDataHandler(
                                events_queue, all_prices_df, all_volumes_df
                            )
                            portfolio = quant_engine.SimplePortfolio(
                                data_handler, events_queue, initial_capital
                            )
                            strategy = quant_engine.SimpleMovingAverageStrategy(
                                data_handler, short_window, long_window
                            )
                            execution_handler = quant_engine.MockExecutionHandler(
                                events_queue, data_handler, portfolio
                            )
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    x=ed_results["equity_curve"].index,
                                    y=ed_results["equity_curve"]["total"],
                                    mode="lines",
                                    name="策略净值",
                                )
                            )
                            ...
                            st.markdown("#### 详细交易记录")
                            st.dataframe(
                                ed_results["trade_log"], use_container_width=True
                            )
...
if tab_trainer:
    with tab_trainer:
        ...
        st.markdown(
            "在这里，您可以选择因子（特征）和预测目标，训练您自己的机器学习选股模型，并将其应用到回测实验室中。"
        )
        ...
        with train_cols[0]:
            train_start_date = st.date_input(
                "训练开始日期", datetime(2023, 1, 1), key="train_start"
            )
        with train_cols[1]:
            train_end_date = st.date_input(
                "训练结束日期", datetime(2024, 12, 31), key="train_end"
            )
        with train_cols[2]:
        ...
        try:
            with data_manager.engine.connect() as conn:
                all_db_factors = pd.read_sql(
                    "SELECT DISTINCT factor_name FROM factors_exposure", conn
                )["factor_name"].tolist()
        except Exception:
            all_db_factors = [
                "momentum",
        ...
        selected_features = st.multiselect(
            "选择用作特征的因子:", options=all_db_factors, default=actual_defaults
        )
            ...
            if not selected_features:
                ...
                with st.spinner("正在执行模型训练工作流，这可能需要几分钟时间..."):
                    try:
                        ...
                        train_end_fetch_str = (
                            train_end_date + timedelta(days=forward_period * 2)
                        ).strftime("%Y%m%d")

                        prices_dict = data_manager.run_batch_download(
                            stock_pool, train_start_str, train_end_fetch_str
                        )
                        all_prices_df = pd.DataFrame(
                            {
                                stock: df.set_index("trade_date")["close"]
                        ...
                        all_factor_data["trade_date"] = pd.to_datetime(
                            all_factor_data["trade_date"]
                        )
                        ...
                        ml_strategy = quant_engine.MLAlphaStrategy()
                        train_results = ml_strategy.train(
                            all_prices_df, all_factor_data
                        )
                        st.success("模型训练成功！")
                        ...
                        st.markdown("#### 训练结果报告")
                        st.metric(
                            "测试集准确率", f"{train_results.get('accuracy', 0):.2%}"
                        )
...
if tab_tasks:
    with tab_tasks:
        ...
        with col1:
            ...
            st.warning(
                "【重要】此任务耗时较长（约20-40分钟），将在后台独立运行。您可以在右侧的日志监控面板查看进度。"
            )

            if st.button(
                "🚀 执行每日统一数据管道 (抽取+计算)",
                help="启动后台进程，执行完整的数据抽取和因子计算流程，并将结果存入数据库。",
            ):
                ...
                try:
                    ...
                    st.error(
                        "错误: 'run_daily_pipeline.py' not found. 请确保该文件与app.py在同一目录下。"
                    )
                except Exception as e:
